...
// src/server/contact-repository.ts
...
function customerContactAckEmail(input) {
  return {
    subject: `We received your message \xB7 ${SITE_NAME}`,
    html: wrapEmail({
      preheader: "Our EU team will reply within 1\u20132 business days.",
      eyebrow: "Contact confirmation",
      title: `Thanks for writing, ${input.name}`,
      intro: `We have received your enquiry about <strong>${escapeHtml2(input.subjectLabel)}</strong>. A member of the team will reply within 1\u20132 business days.`,
      bodyHtml: `
        ...
        <p style="margin:0;font-size:14px;line-height:1.65;color:${EMAIL_BRAND.slate};white-space:pre-wrap;">${escapeHtml2(input.message)}</p>
      `
    }),
    text: `Thanks ${input.name}, we received your ${input.subjectLabel} enquiry and will reply shortly.`
  };
}
function adminContactEmail(input) {
  return {
    subject: `Contact form \xB7 ${input.subjectLabel} \xB7 ${input.name}`,
    html: wrapEmail({
      preheader: `${input.name} submitted a ${input.subjectLabel} enquiry.`,
      eyebrow: "Website enquiry",
      title: "New contact form message",
      intro: `<strong>${escapeHtml2(input.name)}</strong> sent a message via the ${escapeHtml2(SITE_NAME)} contact form.`,
      bodyHtml: `
        ${kvTable([
        ["Name", escapeHtml2(input.name)],
        ["Email", escapeHtml2(input.email)],
        ["Topic", escapeHtml2(input.subjectLabel)]
      ])}
...
async function notifyContactMessage(input) {
...
function parseContactPayload(body) {
  ...
  if (message.length < 10 || message.length > 5e3) return { error: "Please write a message of at least 10 characters" };
