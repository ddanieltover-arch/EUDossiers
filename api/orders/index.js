...
async function listOrders() {
  ...
  `;
  return rows.map(mapRow).filter((order) => Boolean(order)).sort((a2, b2) => Date.parse(b2.createdAt || "") - Date.parse(a2.createdAt || ""));
}
...
async function applyStockChange(productId, warehouseId, quantityChange) {
...
// src/brand.ts
var SITE_NAME = "Eudossier";
var SITE_TAGLINE = "EU commerce & inventory portal";
var CONTACT_EMAIL = "info@eudossier.eu";
var LEGAL_NAME = "Eudossier";
var DOMAIN = "eudossier.eu";
var LEGAL_FOOTER = `\xA9 ${(/* @__PURE__ */ new Date()).getFullYear()} ${LEGAL_NAME} \xB7 Regulation (EU) 2016/679 Compliant.`;
...
function getResendApiKey() {
  ...
  return key && !key.includes("xxxxxxxx") ? key : void 0;
}
...
function decodeParameterValueContinuations(header) {
  ...
  paramKeys.forEach((paramVal, key) => {
    header.params[key] = decodeURIComponentWithCharset(
      paramVal.values.sort((a2, b2) => a2.nr - b2.nr).map((a2) => a2.value).join(""),
      paramVal.charset
    );
  });
}
...
function base64ArrayBuffer(arrayBuffer) {
  ...
  for (var i = 0; i < mainLength; i = i + 3) {
    ...
    c = (chunk & 4032) >> 6;
    d2 = chunk & 63;
    base64 += encodings[a2] + encodings[b2] + encodings[c] + encodings[d2];
  }
  if (byteRemainder == 1) {
    chunk = bytes[mainLength];
    a2 = (chunk & 252) >> 2;
    b2 = (chunk & 3) << 4;
    base64 += encodings[a2] + encodings[b2] + "==";
  } else if (byteRemainder == 2) {
    chunk = bytes[mainLength] << 8 | bytes[mainLength + 1];
    a2 = (chunk & 64512) >> 10;
    b2 = (chunk & 1008) >> 4;
    c = (chunk & 15) << 2;
    base64 += encodings[a2] + encodings[b2] + encodings[c] + "=";
  }
  return base64;
}
...
var AutomationRuns = class {
  ...
  async list(options) {
    ...
    const url = qs2 ? `/automations/${options.automationId}/runs?${qs2}` : `/automations/${options.automationId}/runs`;
    return await this.resend.get(url);
  }
};
...
var Automations = class {
  ...
  async list(options = {}) {
    ...
    const url = qs2 ? `/automations?${qs2}` : "/automations";
    return await this.resend.get(url);
  }
  async get(id) {
...
var Receiving = class {
  ...
  async get(id, options = {}) {
    ...
    return await this.resend.get(path2);
  }
  async list(options = {}) {
...
var Resend = class {
  ...
  async delete(path2, query, options = {}) {
    const headers = new Headers(this.headers);
    if (options.headers) for (const [key, value] of new Headers(options.headers).entries()) headers.set(key, value);
    const requestOptions = {
      method: "DELETE",
...
async function sendBrandedEmail(options) {
  ...
  const to = (Array.isArray(options.to) ? options.to : [options.to]).map((address) => address.trim()).filter(Boolean);
  if (to.length === 0) {
    return { ok: false, error: "No recipient" };
  }
  try {
    const sendOnce = async (withAttachments) => client.emails.send({
      from: getFromAddress(),
      to,
      ...
      attachments: withAttachments && getLogoSrc().attachOgImage ? ogAttachment() : void 0
    });
  ...
  const cta = options.ctaLabel && options.ctaHref ? `
      <tr>
        <td style="padding: 8px 32px 28px;">
          <a href="${escapeHtml2(options.ctaHref)}" style="display:inline-block;background:${EMAIL_BRAND.tealDark};color:${EMAIL_BRAND.white};text-decoration:none;font-weight:700;font-size:14px;padding:12px 22px;border-radius:10px;">
            ${escapeHtml2(options.ctaLabel)}
          </a>
        </td>
      </tr>` : "";
  return `<!DOCTYPE html>
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:${EMAIL_BRAND.white};border-radius:16px;overflow:hidden;border:1px solid ${EMAIL_BRAND.border};">
          <tr>
            <td style="background:${EMAIL_BRAND.ink};padding:0;">
              <img src="${logoSrc}" alt="${escapeHtml2(SITE_NAME)}" width="600" style="display:block;width:100%;max-width:600px;height:auto;border:0;" />
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:28px 32px 8px;">
              <p style="margin:0 0 8px;font-size:11px;letter-spacing:0.14em;text-transform:uppercase;color:${EMAIL_BRAND.tealDark};font-weight:700;">${escapeHtml2(options.eyebrow)}</p>
              <h1 style="margin:0 0 12px;font-size:24px;line-height:1.25;color:${EMAIL_BRAND.ink};font-family:Outfit,Segoe UI,sans-serif;">${escapeHtml2(options.title)}</h1>
          ...
          <tr>
            <td style="padding:20px 32px 28px;border-top:1px solid ${EMAIL_BRAND.border};">
              <p style="margin:0 0 6px;font-size:13px;color:${EMAIL_BRAND.slate};">${escapeHtml2(SITE_NAME)} \xB7 ${escapeHtml2(SITE_TAGLINE)}</p>
              <p style="margin:0;font-size:12px;color:${EMAIL_BRAND.muted};">${escapeHtml2(LEGAL_FOOTER)}<br />${escapeHtml2(DOMAIN)} \xB7 ${escapeHtml2(CONTACT_EMAIL)}</p>
            </td>
          </tr>
        </table>
      </td>
...
function kvTable(rows) {
  const cells = rows.map(
    ([label, value]) => `
        <tr>
          <td style="padding:8px 0;font-size:13px;color:${EMAIL_BRAND.muted};width:38%;vertical-align:top;">${escapeHtml2(label)}</td>
...
function orderItemsHtml(order) {
    ...
    (item) => `
        <tr>
          <td style="padding:10px 0;border-bottom:1px solid ${EMAIL_BRAND.border};">
            <div style="font-size:14px;color:${EMAIL_BRAND.ink};font-weight:700;">${escapeHtml2(item.name)}</div>
            <div style="font-size:12px;color:${EMAIL_BRAND.muted};">SKU ${escapeHtml2(item.sku)} \xB7 Qty ${item.quantity}</div>
          </td>
  ...
  const cryptoRow = order.cryptoDiscountEUR && order.cryptoDiscountEUR > 0 ? `<tr>
...
function orderSummaryHtml(order) {
  return `
    ${kvTable([
    ["Order reference", `<span style="font-family:ui-monospace,Consolas,monospace;">${escapeHtml2(order.id)}</span>`],
    ["Status", escapeHtml2(statusLabel(order.status))],
    ["Customer", escapeHtml2(order.customerName)],
    ["Email", escapeHtml2(order.customerEmail)],
    ["Phone", escapeHtml2(order.customerPhone || "\u2014")],
    ["Address", escapeHtml2(order.customerAddress || "\u2014")],
    ["Destination", escapeHtml2(order.destinationCountry)],
    ["Payment", escapeHtml2(paymentLabel(order.paymentMethod))],
    ["Placed", escapeHtml2(formatDate(order.createdAt))]
  ])}
    <div style="height:16px;"></div>
    ${orderItemsHtml(order)}`;
}
function customerOrderConfirmationEmail(order) {
  const paymentNote = order.paymentMethod === "crypto" ? "Our team will email cryptocurrency payment instructions shortly. A 5% settlement discount is already applied." : `Please complete payment by bank transfer. Quote <strong>${escapeHtml2(order.id)}</strong> as the payment reference. Our team will also follow up at ${escapeHtml2(CONTACT_EMAIL)}.`;
  return {
    subject: `${SITE_NAME} order confirmed \xB7 ${order.id}`,
    html: wrapEmail({
      preheader: `Your ${SITE_NAME} order ${order.id} has been received.`,
      eyebrow: "Order confirmation",
      title: `Thank you, ${order.customerName}`,
      intro: `We have received your order <strong>${escapeHtml2(order.id)}</strong> and it is now <strong>${escapeHtml2(statusLabel(order.status))}</strong>. ${paymentNote}`,
...
function adminNewOrderEmail(order) {
  return {
    subject: `New ${SITE_NAME} order \xB7 ${order.id} \xB7 ${formatEur(order.totalEUR)}`,
    html: wrapEmail({
      preheader: `${order.customerName} placed order ${order.id}.`,
      eyebrow: "Merchant alert",
      title: "New order received",
      intro: `<strong>${escapeHtml2(order.customerName)}</strong> placed an order for <strong>${formatEur(order.totalEUR)}</strong>. Review fulfilment in the admin portal.`,
      bodyHtml: orderSummaryHtml(order)
    }),
...
// src/server/email/notifications.ts
...
async function createCheckoutOrder(input, recordAdjustment) {
  ...
  const order = {
    id: `EU-ORD-${Math.floor(1e4 + Math.random() * 9e4)}`,
    ...
    exchangeRateUsed: input.exchangeRateUsed || 1,
    status: "PROCESSING",
