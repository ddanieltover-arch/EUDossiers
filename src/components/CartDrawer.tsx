        ...
        <div className="w-screen max-w-sm bg-[var(--color-bg-card)] border-l border-[var(--color-border)] text-[var(--color-text-primary)] shadow-2xl flex flex-col">
          ...
          <div className="p-5 border-b border-[var(--color-border)] flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h3 className="font-bold text-[var(--color-text-primary)] text-sm">{t('cart:drawerTitle')}</h3>
                <p className="text-[10px] text-[var(--color-text-muted)]">{t('cart:itemCount', { count: cart.length })}</p>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
            {cart.length === 0 ? (
              <div className="text-center py-12 space-y-2">
                ...
                <p className="text-xs text-[var(--color-text-muted)]">{t('cart:drawerEmpty')}</p>
              </div>
            ) : (
              cart.map(item => (
          ...
          {cart.length > 0 && (
            <div className="p-4 border-t border-[var(--color-border)] space-y-3">
              <div className="flex justify-between items-baseline text-sm">
                <span className="text-[var(--color-text-muted)] text-xs">{t('cart:estimatedTotal')}</span>
