    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-[var(--color-text-muted)]">{cart.length} {cart.length === 1 ? 'item' : 'items'} · EUR (€)</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-3">
          {cart.map(item => {
            ...
            <div key={item.product.id} className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-4 flex items-center space-x-4">
              ...
              <button onClick={() => removeFromCart(item.product.id)} className="text-[var(--color-text-muted)] hover:text-rose-500 p-1.5 rounded-lg transition-colors shrink-0" title={t('remove')}>
        ...
        <div className="lg:col-span-1">
          <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-5 space-y-4 sticky top-24">
            <h3 className="font-bold text-sm text-[var(--color-text-primary)]">{t('summary')}</h3>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex justify-between items-baseline pt-3 border-t border-[var(--color-border)] font-bold">
                <span className="text-[var(--color-text-primary)] text-sm">{t('total')}</span>
            ...
            >
              <span>{tc('actions.proceedCheckout')}</span>
              <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        </div>
