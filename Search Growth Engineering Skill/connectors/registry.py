...
class ConnectorRegistry:
    ...
    def instance(self, connector_id: str) -> Connector:
        ...
        if key not in self._instances:
            ...
            raw = self.config.get('connectors') or {}
            # Workspace configs use connectors as an enabled-id list; per-connector
            # settings (if any) use a dict keyed by connector id.
            if isinstance(raw, list):
                cfg = {}
            else:
                cfg = (raw.get(key) or {}) if isinstance(raw, dict) else {}
